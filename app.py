# app.py – komplett app (svenska) – Chef ser resultat, Medarbetare/Överordnad får tacksida.
...
def survey_core(title: str, instruction: str, questions: List[str], answers_key: str, page_key: str, final_route: str):
    ...
    per = 5
    start_i, end_i = page*per, page*per+per
    for i, q in enumerate(questions[start_i:end_i], start=start_i+1):
        st.markdown(f"**{i}. {q}**")
        radio_row(answers_key, i, ans[i-1])
    ...
    with left:
        if page > 0 and st.button("◀ Tillbaka"):
            ...
            rerun()
    with right:
        ok_page = all(isinstance(v,int) and 1<=v<=7 for v in st.session_state[answers_key][start_i:end_i])
        if page < 3:
...
def assessment():
    ...
    pdf_bytes = build_pdf(PAGE_TITLE, SECTIONS, scores, st.session_state.get("kontakt", {}))
    k = st.session_state.get("kontakt", {})
    pdf_name = f"Självskattning - {k.get('Namn') or 'Person'} - {k.get('Företag') or 'Företag'}.pdf"
    st.download_button("Ladda ner PDF", data=pdf_bytes, file_name=pdf_name,
...
# ─────────────────────────────────────────────────────────
# PDF – identisk 68/32 design (text + resultatkort)
# ─────────────────────────────────────────────────────────
def build_pdf(title: str, sections, results_map, contact: dict) -> bytes:
    ...
    for s in sections:
        ...
        # Topp för båda spalter
        section_top = y

        # Höger – rita kort med tre barer (identisk struktur)
        card_pad = 10
        card_w   = right_w - 10
        per_role = 12 + 10 + 14  # etikett + bar + gap
        ...
        pdf.setFont("Helvetica-Bold", 10); pdf.drawString(inner_x, card_y + card_pad, f"Max: {int(s['max'])} poäng")

        # Vänster – brödtext med wrap inom left_w
