...
EGGSHELL = "#FAF7F0"  # fast äggskalsvit
st.markdown(
    f"""
    <style>
      ...
      /* Resultat-kort */
      .card {{ background: #fff; border-radius: 12px; box-shadow: 0 4px 16px rgba(0,0,0,.08); padding: 14px 16px; border: 1px solid rgba(0,0,0,.12); max-width: 340px; margin: 0 auto; }}
      .summa {{ font-size: 13px; color: #333; margin-top: 6px; }}
      ...
      .role-label {{ font-size: 12px; color: #344054; margin: 6px 0 4px 0; display:block; font-weight:600; }}
...
# Resultat per roll och sektion (dummyvärden – ändra själv eller koppla till logik)
preset_scores = {
...
"):
            st.write(para)
    with right:
        # *** Spacer så resultatkortet hamnar i linje med brödtextens topp (inte rubriken) ***
        ...
        for role in ROLE_ORDER:
            ...
            card_html.append(f"<span class='role-label'>{ROLE_LABELS[role]}: {val} poäng</span>")
            card_html.append(f"<div class='barbg'><span class='barfill {color_class}' style='width:{pct:.0f}%'></span></div>")
        card_html.append(f"<div class='summa'>Max: {maxv} poäng</div>")
...
st.divider()
st.caption("Ladda ner en PDF som speglar allt innehåll – texter och resultat.")

# ---------- PDF (matchar layouten 68/32 och inga tidsstämplar) ----------
...
def build_pdf(title: str, sections, results_map, contact: dict) -> bytes:
    ...
    # Kontakt – två rader (Namn|Företag|Telefon / E-post|Unikt id)
    ...
    for s in sections:
        ...
        # Höger: resultatkort linjerad med brödtextens topp
        card_pad = 10
        card_w   = right_w - 10
        per_role = 12 + 10 + 14
