...
def build_pdf(title: str, sections, results_map, contact: dict) -> bytes:
    ...
    # Kontaktuppgifter – 2 rader i ordningen:
    # Rad 1: Namn | Företag | Telefon
    # Rad 2: E-post | Unikt id
    ...
    pdf.drawString(margin, y, line2); y -= 24   # extra luft under kontakt

    # Liten ytterligare buffert så första H2 hamnar lite lägre
    y -= 6
    ...
    def ensure(px: int):
        ...
        if y - px < 50:
            pdf.showPage()
            paint_bg()
            # H1 på varje ny sida för konsekvent utseende
            yy = h - 60
            pdf.setFont("Helvetica-Bold", 22)
            pdf.drawString(margin, yy, title)
            y = yy - 28

    # Färger för staplar (matchar webben)
    ...
    for s in sections:
        ...
        section_top = y  # textstart för vänsterspalt

        # Höger – kort med 3 progressbars
        card_pad = 10
        card_w   = right_w - 10
        per_role = 12 + 10 + 14  # etikett + bar + mellanrum
        card_h   = card_pad + 3*per_role + 10 + card_pad
        card_y   = section_top - card_h + 6  # lite under H2, i linje med brödtext
